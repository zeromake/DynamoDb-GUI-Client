  ...
  switch (serviceType) {
    case 'remote':
      if (state.submitForm.configs.region === 'cn-north-1' || state.submitForm.configs.region === 'cn-northwest-1') {
        state.submitForm.configs.endpoint = `https://dynamodb.${state.submitForm.configs.region}.amazonaws.com.cn`;
      } else {
      ...
      break;
    case 'local':
      state.submitForm.configs.region = 'ap-east-1';
      state.submitForm.configs.accessKeyId = 'local';
      state.submitForm.configs.secretAccessKey = 'local';
      state.submitForm.name = state.submitForm.name || `Database ${state.list.length + 1}`;
      break;
  }
}
